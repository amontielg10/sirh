...
class ModelCorreoM
{
    function listarById($id_object, $paginator)
    {
        $listado = pg_query("SELECT id_ctrl_medios_contacto_hraes, correo_electronico
                            FROM central.ctrl_medios_contacto_hraes
    ...
    {
        $pg_update = pg_update($conexion, 'central.ctrl_medios_contacto_hraes', $datos, $condicion);
        return $pg_update;
    }

    function agregarByArray($conexion, $datos)
    {
        $pg_add = pg_insert($conexion, 'central.ctrl_medios_contacto_hraes', $datos);
        return $pg_add;
    }

    function eliminarByArray($conexion, $condicion)
    {
        $pgs_delete = pg_delete($conexion, 'central.ctrl_medios_contacto_hraes', $condicion);
        return $pgs_delete;
    }
}
