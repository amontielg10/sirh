...
function iniciarTablaRe(busqueda, paginador) { ///INGRESA LA TABLA
    $.ajax({
        type: 'POST',
        url: '../../../../App/View/Central/Asistencias/Retardos/tabla.php',
        data: {
            busqueda: busqueda,
            paginador: paginador
        },
...
function buscarRetardo() { //BUSQUEDA
    let buscarNew = clearElement(buscar_retardo);
    let buscarlenth = lengthValue(buscarNew);

    if (buscarlenth == 0) {
        iniciarTablaRe(null, iniciarBusqueda_re());
    } else {
        iniciarTablaRe(buscarNew, iniciarBusqueda_re());
    }
}

function mostrarRetardo() {
    $("#modalAgregarRetardo").modal("show");
}

function ocultarRetardo() {
    $("#modalAgregarRetardo").modal("hide");
}
